#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# (c) Shrimadhav U K | Modifieded By : @Minato
...
@Naruto.on_message(filters.private & filters.photo)
async def save_photo(bot, update):
    await AddUser(bot, update)
    await naruto.set_thumbnail(update.from_user.id, thumbnail=update.photo.file_id)
...
@Naruto.on_message(filters.private & filters.command("delthumbnail"))
async def delthumbnail(bot, update):
    await AddUser(bot, update)
    await naruto.set_thumbnail(update.from_user.id, thumbnail=None)
...
@Naruto.on_message(filters.private & filters.command("viewthumbnail") )
async def viewthumbnail(bot, update):
    await AddUser(bot, update)
    thumbnail = await naruto.get_thumbnail(update.from_user.id)
